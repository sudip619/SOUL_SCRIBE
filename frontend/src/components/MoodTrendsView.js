...
const prepareChartData = (logs) => {
  ...
  logs.forEach(log => {
    ...
    if (!dailyAggregates[dateKey]) dailyAggregates[dateKey] = { sumWellbeing: 0, sumEnergy: 0, count: 0 };
...
export default function MoodTrendsView({ showAlert, onOpenGraph }) {
  ...
  const renderChart = useCallback((canvasElement, chartRefObject, labels, wellbeingDataPoints, energyDataPoints) => {
    ...
    if (chartRefObject.current) chartRefObject.current.destroy();
          ...
          tooltip: {
            callbacks: {
              label: function (context) {
                ...
                if (label) label += ': ';
                return label + (context.raw != null ? context.raw.toFixed(1) : '');
              }
            }
          }
        }
    ...
    chartRefObject.current = newChartInstance;
  ...
  const renderStackedBarChart = useCallback((canvasElement, chartRefObject, labels, datasets) => {
    ...
    const newChartInstance = new Chart(ctx, {
      ...
      options: {
        ...
        layout: { padding: { left: 20, right: 20, top: 10, bottom: 10 } },
        scales: {
          x: { stacked: true, title: { display: true, text: 'Date', color: '#E0E0E0', font: { size: 16, weight: 'bold' } }, ticks: { color: '#E0E0E0' }, grid: { color: 'rgba(136,136,136,0.2)' } },
          y: { stacked: true, beginAtZero: true, title: { display: true, text: 'Number of Moods Logged', color: '#E0E0E0', font: { size: 16, weight: 'bold' } }, ticks: { stepSize: 1, color: '#E0E0E0', padding: 10 }, grid: { color: 'rgba(136,136,136,0.2)' } }
        },
        plugins: {
  ...
  useEffect(() => {
    const loadMoodTrends = async () => {
      try {
        const { data, error } = await supabase.from('mood_logs').select('mood_name, timestamp').order('timestamp', { ascending: true });
        ...
        showAlert && showAlert(error.message || 'Failed to load mood trends.', false);
      }
    };
      ...
      {moodLogs.length === 0 ? (
        ...
        <>
          ...
          <div>
            ...
            <div className="chart-wrapper w-full overflow-x-auto p-4 rounded-lg shadow-inner panel-surface">
              <canvas ref={stackedBarChartRef} className="w-full h-[400px]" />
            </div>
          </div>
